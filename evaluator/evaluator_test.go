...
func testIntegerObject(t *testing.T, obj object.Object, expect int64) bool {
	...
	if result.Value != expect {
		t.Errorf("obj has wrong value. got=%d, want=%d",
			obj, expect)
		return false
	}
...
func testBooleanObject(t *testing.T, obj object.Object, expect bool) bool {
	...
	if result.Value != expect {
		t.Errorf("obj has wrong value. got=%t, want=%t",
			obj, expect)
		return false
	}
...
func TestFunctionObject(t *testing.T) {
	...
	if !ok {
		t.Fatalf("evaluated is not Function. got=%T(%+v)", evaluated, evaluated)
	}
...
func TestBuiltinFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		case string:
			errObj, ok := evaluated.(*object.Error)
			if !ok {
				t.Errorf("object s not Error. got=%T (%+v)",
					evaluated, evaluated)
				continue
			}
...
func TestArrayLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("evaluated is not Array. got=%T (%+v)",
			evaluated, evaluated)
	}
